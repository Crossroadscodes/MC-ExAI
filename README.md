...
### v1.0.3 (2026-05-28)
- **新增公屏问答自动采集**：监听「提问 → 回答 →（致谢）」对话流，AI 初审后自动入队待审核
  - 新增 `com.exai.manager.ChatKnowledgeCollector`、`com.exai.service.KnowledgeReviewService`、`com.exai.entity.ReviewResult`
  - 新增配置组 `knowledge.autoCollect`（开关 / 回答窗口 / 感谢窗口 / 最小回答长度 / 审核员提示 / 感谢关键词）
- **优化公屏回复处理逻辑，防止刷屏**：AI 无依据可答时（命中拒答标记）不再广播"找不到相关信息"
- **优化提示词，降低幻觉率**：
  - 强约束「仅依据知识库作答、无依据则拒答」，并将最强约束置于 prompt 结尾
  - 新增 `llm.temperature`（默认 0.3）与 `knowledge.maxDocs`（默认 3）
  - GUI 与公屏统一走严格 grounding 模板
- **玩家书本提交新增 AI 初审**（`knowledge.playerSubmitReview`）：不合规内容直接拦截，AI 不可用时 fail-closed
...
│   └── ReviewResult.java        # ★ v1.0.3 AI 初审结果实体
├── generators/
...
├── manager/
│   ├── ChatKnowledgeCollector.java  # ★ v1.0.3 公屏问答自动采集
...
├── service/
│   ├── KnowledgeReviewService.java  # ★ v1.0.3 知识 AI 初审
